...
while (
  personpersonName === undefined ||
  personName === null ||
  personName.length === 0
) {
  personName = prompt("Hoe heet je?");
}

alert("Welkom bij Guess the Number " + personName + "!!");

max = parseInt(prompt("Tot welk nummer mag er geraden worden?"));

alert(
  "Je hebt nu 5 pogingen om het getal te raden. Het getal ligt tussen 0 en " +
    max +
    ". Heel veel succes!"
);
...
while (guess !== numberToBeGuessed) {
  if (numberOfGuesses < 1) {
    alert(
      "Helaas, je pogingen zijn op. Je hebt verloren. We beginnen weer opnieuw met een nieuw nummer! Het nummer ligt nog steeds tussen 0 en " +
        max +
        "."
    );
    numberOfGuesses = 5;
